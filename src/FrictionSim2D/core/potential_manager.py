...
for cross-interactions, and layer-specific
potential handling for multi-layer 2D materials.
...
class PotentialManager:
    ...
    def __init__(self, settings: GlobalSettings, use_langevin: Optional[bool] = None):
        """Initialize the PotentialManager.
        
        Args:
            settings: Global simulation settings containing potential and thermostat config.
            use_langevin: Override for Langevin type expansion. If None, uses settings. 
                          Set to False for temporary calculations like lat_c finding.
        """
    ...
    def register_component(
        ...
        """Registers a material component and assigns unique global atom type IDs.
    ...
    def _assign_standard_types(
        ...
        """Assigns standard atom types (one set per component)."""
        component_map = {}
        type_index = 1
    ...
    def _assign_layer_types(
        ...
        """Assigns atom types for multi-layer 2D materials.
    ...
    def _assign_langevin_types(
        ...
        """Assigns types for Langevin thermostat (3 regions per element)."""
        component_map = defaultdict(list)
        type_index = 1
    ...
    def is_sheet_lj(self, pot_type: str) -> bool:
        """Determines if the potential type requires explicit LJ for interlayer/cross interactions.
    ...
    def add_self_interaction(self, component_name: str, layer: Optional[int] = None):
        """Adds the self-interaction (many-body potential) for a component.
    ...
    def _add_layer_self_interaction(self, comp: Dict, layer: int, base_pot_index: int):
        """Adds self-interaction for a specific layer."""
        c_conf = comp['config']
        name = comp['name']
    ...
    def _build_null_map(self, component_name: str) -> List[str]:
        """Builds the NULL-padded element list for a component's pair_coeff.
    ...
    def add_interlayer_interaction(
        ...
        """Adds LJ interactions between layers of a multi-layer 2D material.
        
        Args:
            component_name: Name of the sheet component.
            layer_pairs: List of (layer_i, layer_j) tuples, or None for all pairs.
            low_interaction: If True, use near-zero epsilon (ghost/weak interaction).
        """
        comp = self._get_component(component_name)
        n_layers = comp['n_layers']
    ...
    def add_interlayer_lj_by_distance(
        ...
        """Adds all interlayer LJ interactions with distance-based ghost handling.
        
        Layer pairs within max_real_distance get real LJ interactions.
        Layer pairs beyond max_real_distance get ghost (near-zero epsilon) interactions
        to prevent interpenetration without affecting dynamics significantly.
        
        Args:
            component_name: Name of the multi-layer component.
            max_real_distance: Maximum layer separation for real LJ interactions.
                - 1 (default): Only adjacent pairs (L1-L2, L2-L3, L3-L4) are real.
                - 2: Adjacent + next-nearest (L1-L3, L2-L4) are real.
                - n_layers or higher: All pairs are real (no ghost interactions).
    ...
    def _get_element_type_range(self, comp_name: str, element: str) -> str:
        """Gets the LAMMPS type range string for an element in a component.
        
        Returns format like '1*3' for types 1,2,3 or just '5' for single type.
        """
        all_types = []
    ...
    def _format_type_range(self, types: List[int]) -> str:
        """Formats a list of type IDs into LAMMPS range notation."""
        if not types:
            return ""
        types = sorted(types)
        if len(types) == 1:
    ...
    def calculate_gap(self, comp1_name: str, comp2_name: str, buffer: float = 0.5) -> float:
        """Calculates the recommended gap between two components.
    ...
    def get_masses_string(self) -> str:
        """Generates LAMMPS mass commands for all registered types.
    ...
    def get_layer_groups_string(self) -> str:
        """Generates LAMMPS group commands for multi-layer components only.
    ...
    def write_file(self, output_path: Path):
        """Writes the complete potential settings file.
    ...
    def get_group_string(self, component_name: str) -> str:
        """Returns space-separated type IDs for a component (for LAMMPS grouping).
    ...
    def get_layer_group_string(self, component_name: str, layer: int) -> str:
        """Returns type IDs for a specific layer of a component.
    ...
    def get_total_types(self) -> int:
        """Returns the total number of atom types registered."""
