...
class AFM_simulation(model_init.model_init):
    ...
    def afm_potentials_setup(self,layer):
        ...
        with open(filename, 'w') as f:
            ...
            for system in self.systems:     
                all = [self.group_def[i][1] for i in range(1,self.ngroups[layer]+1) if system in self.group_def[i][0]]
                f.write(f"group {system}_all type {' '.join(all)}\n")   
                if system == '2D':
                    for l in range(layer):
                        layer_g = [self.group_def[i][1] for i in range(1,self.ngroups[layer]+1) if "2D_l"+str(l+1) in self.group_def[i][0]]
                        f.write(f"group layer_{l+1} type {' '.join(layer_g)}\n")
                else:
                    for n in ["_fix", "_thermo"]:
                        sub_group = [self.group_def[i][1] for i in range(1,self.ngroups[layer]+1) if system+n in self.group_def[i][0]]
                        f.write(f"group {system}{n} type {' '.join(sub_group)}\n")
            ...
            for system in self.systems:
                ...
                if system == '2D':
                    for l in range(layer):
                        ...
                        f.write(f"pair_coeff * * {self.params['2D']['pot_type']} {t+l} {self.potentials['2D']['path']} {'  '.join(potentials[l])} # interlayer '2D' Layer {l+1}\n")
                else:
                    potentials[system]=[self.group_def[i][2] if system in self.group_def[i][0] else "NULL"
                                    for i in range(1,self.ngroups[layer]+1)]
                    f.write(f"pair_coeff * * {self.params[system]['pot_type']} {t} {self.potentials[system]['path']} {'  '.join(potentials[system])} # interlayer {system.capitalize()}\n")
                ...
                for key in ('sub','tip'):
                    for s in self.data[key]['elem_comp']:
                        ...
                        if len(self.elemgroup['2D'][layer-1][t]) == 1 and layer == 1:
                            f.write(f"pair_coeff {self.elemgroup[key][s][0]}*{self.elemgroup[key][s][-1]} {self.elemgroup['2D'][0][t][0]} lj/cut {e} {sigma}\n")
                        else:  
                            f.write(f"pair_coeff {self.elemgroup[key][s][0]}*{self.elemgroup[key][s][-1]} {self.elemgroup['2D'][0][t][0]}*{self.elemgroup['2D'][layer-1][t][-1]} lj/cut {e} {sigma}\n")
            if layer>1:
            ...
            for s in self.data['sub']['elem_comp']:
                for t in self.data['tip']['elem_comp']:
                    ...
                    f.write(f"pair_coeff {self.elemgroup['sub'][t][0]}*{self.elemgroup['sub'][t][-1]} {self.elemgroup['tip'][t][0]}*{self.elemgroup['tip'][t][-1]}  lj/cut {e} {sigma} \n")
    ...
    def __single_body_3layer(self,filename,system):
        ...
        with open(filename, 'w') as f:
            ...
            potentials = [self.group_def[i][2] for i in range(1,self.data[system]['natype']*3+1)]
    ...
    def __define_elemgroup_3regions(self,system,arr,atype=1):
        ...
        for element, count in self.potentials[system]['count'].items():
            for _ in range(1,count+1):
                ...
                self.elemgroup.setdefault(system, {}).setdefault(element, []).extend([atype, atype+1, atype+2]) 
                i+= 1
                atype+=3
