"""
This set of functions is used to build and modify different components
of molecular dynamics simulations, specifically for systems based on
the Prandtl-Tomlinson model and the study of 2D materials.
...
from tribo_2D import tools, potentials
...
def tip(var):
    ...
    pot_file = f"{var['dir']}/system_build/tip.in.settings"
    potentials.single_body_3layer(var, pot_file, 'tip')

    x = 2 * var['tip']['r']
    z = var['tip']['r']

    # Generate a slab for the tip of either amoprhous or crystalline material
    if var['tip']['amorph'] == 'a':
        ...
        filename = os.path.join(
            os.path.dirname(__file__), "materials", f"{var['tip']['mat']}.lmp")
        dim = tools.get_model_dimensions(filename)
        dim_x = dim['xhi']/2

    h = dim_x / 2.25  # Define region height
...
def sub(var):
    ...
    pot_file = f"{var['dir']}/system_build/sub.in.settings"
    potentials.single_body_3layer(var, pot_file, 'sub')
    ...
    if var['sub']['amorph'] == 'a':
        ...
        filename = os.path.join(os.path.dirname(
            __file__), "materials", f"{var['sub']['mat']}.lmp")
    ...
    # Assign atom types and groups for substrate, fix, and thermostat regions
...
def amorph(system, filename, am_filename, tempmelt, var):
    ...
    pot_file = f"{var['dir']}/system_build/{system}.in.settings"
    potentials.single_body(var, pot_file, system)
    lmp = lammps(cmdargs=["-log", "none", "-screen", "none",  "-nocite"])
    lmp.commands_list([
        "boundary p p p\n",
        "units metal\n",
...
def sheet(var):
    ...
    if first_multiple is None:
        ...
        raise ValueError(
            'potential file or cif file is not formatted properly')

    else:
        if first_multiple == 1:
            atomsk_command = f"echo n | atomsk {var['2D']['cif_path']} -ow {filename} -v 0"
            subprocess.run(atomsk_command, shell=True, check=True)
            tools.num_atoms_lmp(filename)
            atomsk_command = f"atomsk {filename} -orthogonal-cell -ow lmp -v 0"
            subprocess.run(atomsk_command, shell=True, check=True)
            ...
            subprocess.run(atomsk_command, shell=True, check=True)
            tools.process_h(var['pot']['2D'], filename)
    ...
    var['dim'] = tools.get_model_dimensions(filename)
...
def stacking(var, layer=2, sheetvsheet=False):
    ...
    potentials.sheet(
        var, f"{var['dir']}/system_build/sheet_{layer}.in.settings", layer)
    ...
    if sheetvsheet:
        for l in range(1, 4):
            lmp.command(
                f"read_data {filename}_1.lmp add append shift 0 0 {layer*lat_c} group layer_{layer+1}\n")
        lmp.commands_list([
    ...
    for count in var['pot']['2D'].values():
        i += c
...
def slab_generator(system, var, x, y, z):
    filename = os.path.join(os.path.dirname(__file__),
                            "materials", f"{var[system]['mat']}.lmp")
    Path(filename).unlink(missing_ok=True)
